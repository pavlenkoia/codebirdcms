        ...
        [
                        ...
                        [
                            {
                                ...
                                handler: function()
                                {
                                    ...
                                    {
                                        form.getForm().submit({
                                            url: '/ajax/cm/infoblock.cm.add',
                                            method: 'POST',
                                            waitTitle: 'Подождите',
                                            waitMsg: 'Создание инфоблока...',
                                            success: function(form, action)
                                            {
                                                win.hide();
                                                var tree = Ext.getCmp('<?php echo $args->module ?>-navigator-tree');
                                                tree.getLoader().load(tree.root);
                                                App.msg('Готово', 'Инфоблок создан');
                                            },
            ...
            },
            {
                ...
                handler: function(item)
                {
                    ...
                    Ext.MessageBox.confirm('Подтверждение', 'Вы&nbsp;действительно&nbsp;хотите&nbsp;удалить&nbsp;инфоблок:&nbsp;'+node.text+'?',
                        function(btn)
                        {
                            if(btn == 'yes')
                            {
                                Ext.Ajax.request(
                                    {
                                        url : '/ajax/cm/infoblock.cm.delete',
                                        method: 'POST',
                                        params:
                                        {
                                            id: node.id
                                        ...
                                        success : function (response)
                                        {
                                            ...
                                            if(obj.success)
                                            {
                                                ...
                                                tree.getLoader().load(tree.root);
                                                App.msg('Готово', 'Инфоблок удален');
        ...
        <?php if($args->top) echo 'render'; else echo 'expand' ?> : function(panel,anim)
        {
            if(panel.firstexp != true)
            {
                var tree = new Ext.tree.TreePanel({
                    ...
                    autoScroll: true,
                    rootVisible: false,
                    dataUrl: '/ajax/cm/infoblock.cm.tree',
                    ...
                    listeners :
                    {
                        click : function(node)
                        {
                            ...
                            if(node.id > 0)
                            {
                                dis = false;
                                App.showEditor({
                                    url: '/ajax/cm/infoblock.cm.editor?id='+node.id,
                                    id : '<?php echo $args->module ?>-edit-'+node.id,
                                    caption: 'Инфоблок: '+node.text
                                    });
                            }
