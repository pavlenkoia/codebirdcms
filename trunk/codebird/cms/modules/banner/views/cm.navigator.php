        ...
        [
                        ...
                        [
                            {
                                ...
                                handler: function()
                                {
                                    ...
                                    {
                                        form.getForm().submit({
                                            url: '/ajax/cm/banner.cm.add',
                                            method: 'POST',
                                            waitTitle: 'Подождите',
                                            waitMsg: 'Создание баннера...',
                                            success: function(form, action)
                                            {
                                                win.hide();
                                                var tree = Ext.getCmp('<?php echo $args->module ?>-navigator-tree');
                                                tree.getLoader().load(tree.root);
                                                App.msg('Готово', 'Баннер создан');
                                            },
            ...
            },
            {
                ...
                handler: function(item)
                {
                    ...
                    Ext.MessageBox.confirm('Подтверждение', 'Вы&nbsp;действительно&nbsp;хотите&nbsp;удалить&nbsp;баннер:&nbsp;'+node.text+'?',
                        function(btn)
                        {
                            if(btn == 'yes')
                            {
        ...
        <?php if($args->top) echo 'render'; else echo 'expand' ?> : function(panel,anim)
        {
            if(panel.firstexp != true)
            {
                var tree = new Ext.tree.TreePanel({
                    ...
                    autoScroll: true,
                    rootVisible: false,
                    dataUrl: '/ajax/cm/banner.cm.tree',
                    ...
                    listeners :
                    {
                        click : function(node)
                        {
                            ...
                            if(node.id > 0)
                            {
                                dis = false;
                                App.showEditor({
                                    url: '/ajax/cm/banner.cm.editor?id='+node.id,
